...
### 注意事项一定要读 
1. 航信这个系统设备密码一般税务局那边会告诉你，角色密码默认是空，就是可以直接登录进去。  

2. 登录进去的第一件事就是添加操作员并以新的操作员登录，尽量不要开落款带有管理员的票，即使这并不影响发票报销。

3. 如果你用A角色开票，B角色的开票列表里是看不到的，但管理员可以看到所有的票，管理员开的票只有管理员能看到。

4. 不要没事开太多负数发票，税务会找麻烦。